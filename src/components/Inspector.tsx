...
import { Settings, Zap, TrendingUp, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';
...
const PRESETS = {
  channel: {
    facebook: { name: '페이스북 광고', cpc: 800, ctr: 1.2, dailyBudget: 50000 },
    instagram: { name: '인스타그램 광고', cpc: 900, ctr: 1.8, dailyBudget: 50000 },
    google: { name: '구글 검색 광고', cpc: 1200, ctr: 2.5, dailyBudget: 100000 },
    naver: { name: '네이버 광고', cpc: 600, ctr: 1.0, dailyBudget: 50000 },
  },
};
...
export function Inspector({ selectedNode, onUpdateNode }: InspectorProps) {
  ...
  const renderFields = () => {
    switch (selectedNode.type) {
      ...
      case 'channel':
        ...
        return (
          <>
            {/* Preset Buttons */}
            <div className="mb-4">
              <label className="block text-neutral-400 text-xs mb-2">빠른 설정</label>
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(PRESETS.channel).map(([key, preset]) => (
                  <button
                    key={key}
                    onClick={() => applyPreset(preset)}
                    className="px-3 py-2 bg-neutral-800 hover:bg-blue-600 text-neutral-300 hover:text-white rounded text-xs transition-colors"
                  >
                    {preset.name}
                  </button>
                ))}
            ...
            <div>
              <label className="block text-neutral-400 text-xs mb-2">채널 유형</label>
              <select
              ...
              >
                ...
                <option value="organic">오가닉/SEO</option>
            ...
            <div className="pt-4 border-t border-neutral-800">
              ...
              <FormField
                ...
                hint={`예상 일일 유입: ${Math.floor(selectedNode.data.dailyBudget / selectedNode.data.cpc)}명`}
              />
              ...
              <div>
                <label className="block text-neutral-400 text-xs mb-2">휘발성</label>
                <select
                  value={selectedNode.data.volatility}
                  onChange={(e) => handleChange('volatility', e.target.value)}
                  className="w-full px-3 py-2 bg-neutral-800 border border-neutral-700 rounded text-white text-sm focus:outline-none focus:border-blue-500"
                >
                  <option value="high">High - 광고 중단 시 즉시 0</option>
      ...
      case 'landing':
      case 'signup':
        const warningConv = getBenchmarkWarning('conversionRate', selectedNode.data.conversionRate);
          ...
          <>
            ...
            <div className="pt-4 border-t border-neutral-800">
              ...
              <SliderField
                ...
                step={0.1}
                warning={warningConv}
                hint={`이탈률: ${(100 - selectedNode.data.conversionRate).toFixed(1)}%`}
              />
              
              <div>
                <label className="block text-neutral-400 text-xs mb-2">복잡도</label>
                <select
                ...
                {selectedNode.data.complexity === 'high' && (
                  <p className="text-yellow-400 text-xs mt-1">⚠️ 전환율 -20% 패널티</p>
                )}
              </div>
            </div>
          </>
      ...
      case 'payment':
        ...
        const marginRate = selectedNode.data.price > 0 ? (margin / selectedNode.data.price * 100) : 0;
          ...
          <>
            ...
            <div className="pt-4 border-t border-neutral-800">
              ...
              <FormField
                ...
                hint={`마진: ₩${margin.toLocaleString()} (${marginRate.toFixed(1)}%)`}
              />
      ...
      case 'subscription':
        const warningChurn = getBenchmarkWarning('churnRate', selectedNode.data.churnRate);
        const avgLifetime = selectedNode.data.churnRate > 0 ? (100 / selectedNode.data.churnRate).toFixed(1) : '∞';
        const ltv = selectedNode.data.churnRate > 0 
          ? (selectedNode.data.monthlyPrice * (100 / selectedNode.data.churnRate)).toFixed(0)
          : '∞';
          ...
          <>
            ...
            <div className="pt-4 border-t border-neutral-800">
              ...
              <div className="p-3 bg-green-950/30 border border-green-900 rounded mt-3">
                ...
                <p className="text-green-300 text-lg">₩{typeof ltv === 'string' ? ltv : Number(ltv).toLocaleString()}</p>
              </div>
            </div>
          </>
        );
    ...
    <aside className="w-80 bg-neutral-900 border-l border-neutral-800 overflow-y-auto">
      <div className="p-6">
        ...
        <div className="mb-4 pb-4 border-b border-neutral-800">
          <span className="text-xs text-neutral-500 uppercase tracking-wide">블록 타입</span>
        ...
        <div className="mt-6 p-3 bg-neutral-800 rounded text-xs text-neutral-400 leading-relaxed">
          <strong className="text-neutral-300">Tip:</strong> 슬라이더를 움직이면 실시간으로 시뮬레이션이 업데이트됩니다.
        </div>
      </div>
    </aside>
  );
...
function FormField({ label, value, onChange, type = 'text', placeholder, step, multiline, hint }: FormFieldProps) {
  return (
    <div>
...
function SliderField({ label, value, onChange, min, max, step, hint, warning, feedback }: SliderFieldProps) {
  const [recentChange, setRecentChange] = React.useState<string | null>(null);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-white text-sm font-semibold">{value.toFixed(step >= 1 ? 0 : 1)}</span>
      </div>
      <Slider
        value={[value]}
      ...
      {hint && !warning && !recentChange && <p className="text-neutral-500 text-xs mt-1">{hint}</p>}
    </div>
  );
}