...
export function NodeComponent({
  ...
  const getNodeStatus = (): {
    status: "complete" | "warning" | "danger" | "inactive";
    color: string;
    label: string;
  } => {
  ...
  const renderKeyMetrics = () => {
    ...
    if (traffic > 0) {
      metrics.push(
        <div
          key="traffic"
          className="flex items-center justify-between px-3 py-1.5 bg-neutral-900/50 rounded"
        >
    ...
    if (node.data.dailyBudget) {
      metrics.push(
        <div
          key="budget"
          className="flex items-center justify-between px-3 py-1.5 bg-orange-950/30 rounded border border-orange-900/50"
        >
          <span className="text-xs text-orange-300 flex items-center gap-1">
            <DollarSign className="w-3 h-3" />일 예산
          </span>
          <span className="text-sm text-orange-200 font-bold">
            ₩{(node.data.dailyBudget / 10000).toFixed(0)}만
          </span>
        </div>
      );
    }
    ...
    if (node.data.price) {
      metrics.push(
        <div
          key="price"
          className="flex items-center justify-between px-3 py-1.5 bg-green-950/30 rounded border border-green-900/50"
        >
    ...
    if (node.data.monthlyPrice) {
      metrics.push(
        ...
        >
          <span className="text-xs text-green-300 flex items-center gap-1">
            <DollarSign className="w-3 h-3" />월 구독료
          </span>
          <span className="text-sm text-green-200 font-bold">
            ₩{node.data.monthlyPrice.toLocaleString()}
          </span>
        </div>
      );
    }
    ...
    >
      ...
      >
        ...
        <div className="px-3 py-3 space-y-2">
          <p className="text-white font-medium text-sm px-1">
            {node.data.name}
          </p>

          {/* Key Metrics Display */}
          {keyMetrics.length > 0 && (
            <div className="space-y-1.5">{keyMetrics}</div>
          )}
      ...
      {/* Input connector */}
      <button
        className="node-action absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-neutral-800 border-2 border-white/60 shadow-[0_0_10px_rgba(255,255,255,0.4)] hover:border-cyan-300 hover:bg-cyan-600 hover:shadow-[0_0_14px_rgba(34,211,238,0.7)] hover:scale-125 transition-all z-30"
        onClick={handleInputClick}
        title="입력 연결"
      >
        <Circle className="w-3 h-3 text-white/80" />
      </button>

      {/* Output connector */}
      <button
        className="node-action absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-neutral-800 border-2 border-white/60 shadow-[0_0_10px_rgba(255,255,255,0.4)] hover:border-cyan-300 hover:bg-cyan-600 hover:shadow-[0_0_14px_rgba(34,211,238,0.7)] hover:scale-125 transition-all z-30"
